...
class _InjectionScheduleState extends State<InjectionSchedule> {
  ...
  void initState() {
    ...
    childViewModel.getChildByID(CurrentMember.pregnancyFlag == true ? CurrentMember.pregnancyID : CurrentMember.id);

    injectionScheduleViewModel = InjectionScheduleViewModel.getInstance();
    injectionScheduleViewModel.getInjectionSchedule(CurrentMember.pregnancyFlag == true ? CurrentMember.pregnancyID : CurrentMember.id);
  ...
  Widget ChildInfo(ChildModel childModel){
    return ListTile(
        leading: childModel == null
          ...
          radius: 22,
          backgroundColor: LIGHT_GREY_COLOR,
          backgroundImage: CachedNetworkImageProvider(childModel.imageURL),),
        title: Text(childModel == null ? "..." : childModel.fullName,style: TextStyle(fontWeight: FontWeight.w600),));
  }
  ...
  Widget HeaderItem(num size, String name){
    return Container(
      ...
      child: Center(
        child: Text(
          name,
          style: TextStyle(color: WHITE_COLOR,fontSize: 16,fontWeight: FontWeight.w600)
        ),
      ),
    );
  }
