...
import 'package:mumbi_app/modules/health_history/repositories/pregnancy_history_repository.dart';
...
class PregnancyHistoryViewModel extends Model {
  ...
  void getPregnancyHistory(String childId, String date) async {
    try {
      var data = await PregnancyHistoryRepository.apiGetPregnancyHistory(
          childId, date);
      Map<String, dynamic> jsonList = json.decode(data);
      pregnancyHistoryList = jsonList['data'];
      if (pregnancyHistoryList != null) {
        pregnancyHistoryListModel = pregnancyHistoryList
            .map((e) => PregnancyHistoryModel.fromJson(e))
            .toList();
        if (date != "") {
  ...
  Future<bool> updatePregnancyHistory(String childId,
      PregnancyHistoryModel pregnancyHistoryModel, String date) async {
    try {
      String result =
          await PregnancyHistoryRepository.apiUpdatePregnancyHistory(
              childId, pregnancyHistoryModel, date);
      if (result != null) {
        return true;
      } else {
        return false;
