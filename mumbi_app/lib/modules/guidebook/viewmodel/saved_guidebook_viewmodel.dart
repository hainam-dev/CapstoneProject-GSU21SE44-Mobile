...
import 'package:mumbi_app/modules/guidebook/repositories/saved_guidebook_repository.dart';
...
class SavedGuidebookViewModel extends Model {
  ...
  void getSavedGuidebook() async {
    try {
      ...
      String data =
          await SavedGuidebookRepository.apiGetSavedGuidebook(momId, 1);
      Map<String, dynamic> jsonList = json.decode(data);
      List<dynamic> savedGuidebookList = jsonList['data'];
      if (savedGuidebookList != null) {
        savedGuidebookListModel = savedGuidebookList
            .map((e) => SavedGuidebookModel.fromJson(e))
            .toList();
      }
  ...
  void getMoreSavedGuidebook() async {
    try {
      if (currentPage < totalPage) {
        isLoading = true;
        String momId = await UserViewModel.getUserID();
        String data = await SavedGuidebookRepository.apiGetSavedGuidebook(
            momId, ++currentPage);
        Map<String, dynamic> jsonList = json.decode(data);
        List<dynamic> savedGuidebookList = jsonList['data'];
        if (savedGuidebookList != null) {
          List<SavedGuidebookModel> moreSavedGuidebookListModel =
              savedGuidebookList
                  .map((e) => SavedGuidebookModel.fromJson(e))
                  .toList();
          currentPage = jsonList['pageNumber'];
  ...
  Future<bool> saveGuidebook(String newsId) async {
    ...
    try {
      String result =
          await SavedGuidebookRepository.apiSaveGuidebook(savedGuidebookModel);
      if (result != null) {
        return true;
      } else {
        return false;
  ...
  Future<bool> unsavedGuidebook(SavedGuidebookModel savedGuidebookModel) async {
    try {
      String result = await SavedGuidebookRepository.apiUnsavedGuidebook(
          savedGuidebookModel.id);
      if (result != null) {
        return true;
      } else {
        return false;
