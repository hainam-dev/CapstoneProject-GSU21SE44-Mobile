...
import 'package:mumbi_app/modules/community/model/reaction_model.dart';
import 'package:mumbi_app/modules/community/repositories/reaction_repository.dart';
...
class ReactionViewModel extends Model {
  ...
  void getPostReaction(num postId) async {
    try {
      ...
      reactionListModel =
          reactionList.map((e) => ReactionModel.fromJson(e)).toList();
      notifyListeners();
    } catch (e) {
  ...
  void checkPostReaction(PostModel postModel) async {
    ...
    var checkReact =
        await ReactionRepository.apiCheckPostReaction(userId, postModel.id);
    List checkReactJsonList = json.decode(checkReact)['data'];
    if (checkReactJsonList != null) {
      List reactionModelList =
          checkReactJsonList.map((e) => ReactionModel.fromJson(e)).toList();
  ...
  void checkCommentReaction(CommentModel commentModel) async {
    String userId = await UserViewModel.getUserID();
    var checkReact = await ReactionRepository.apiCheckCommentReaction(
        userId, commentModel.id);
    List checkReactJsonList = json.decode(checkReact)['data'];
    if (checkReactJsonList != null) {
      List reactionModelList =
          checkReactJsonList.map((e) => ReactionModel.fromJson(e)).toList();
  ...
  Future<bool> addPostReaction(num postId) async {
    ...
    try {
      String result =
          await ReactionRepository.apiAddPostReaction(reactionModel);
      if (result != null) {
        return true;
      } else {
        return false;
  ...
  Future<bool> addCommentReaction(num commentId) async {
    ...
    try {
      String result =
          await ReactionRepository.apiAddCommentReaction(reactionModel);
      if (result != null) {
        return true;
      } else {
        return false;
