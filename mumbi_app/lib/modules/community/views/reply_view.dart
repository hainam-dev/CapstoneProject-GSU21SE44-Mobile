    ...
    return Scaffold(
      body: Scaffold(
        ...
        bottomNavigationBar: Container(
          ...
          child: Card(margin: EdgeInsets.zero, child: CommentSection()),
        ),
      ),
    );
  }
  ...
  Widget UserComment() {
    return Container(
      ...
      child: CommentTreeWidget<Comment, Comment>(
        ...
        [],
        treeThemeData:
            TreeThemeData(lineColor: TRANSPARENT_COLOR, lineWidth: 0.5),
                ...
                child: Padding(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      Text(
                          "${DateTimeConvert.timeAgoInShort(widget.commentModel.createdTime)}"),
                      SizedBox(
                        width: 15,
                      ),
                      Text(
                        widget.commentModel.totalReaction == 0
                            ? ""
                            : widget.commentModel.totalReaction.toString(),
                        style: TextStyle(
                            color: widget.commentModel.idReaction != 0
                                ? PINK_COLOR
                                : LIGHT_DARK_GREY_COLOR),
                      ),
                      SizedBox(
                        width: 2,
                      ),
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (widget.commentModel.idReaction != 0) {
                            await ReactionViewModel()
                                .deleteReaction(widget.commentModel.idReaction);
                            await ReactionViewModel()
                                .checkCommentReaction(widget.commentModel);
                            widget.commentModel.totalReaction--;
                          } else {
                            await ReactionViewModel()
                                .addCommentReaction(widget.commentModel.id);
                            await ReactionViewModel()
                                .checkCommentReaction(widget.commentModel);
                            widget.commentModel.totalReaction++;
                          }
                      ...
                      Text(
                          "${widget.commentModel.totalReply == 0 ? "" : widget.commentModel.totalReply.toString()} Trả lời"),
                      SizedBox(
                        width: 15,
                      ),
                      if (widget.commentModel.userId ==
                          _momViewModel.momModel.id)
                        GestureDetector(
                          onTap: () {
                            showConfirmDialog(context, "Xoá",
                                "Bạn có muốn xóa bình luận này?",
                                ContinueFunction: () async {
                              Navigator.pop(context);
                              showProgressDialogue(context);
                              bool result = await CommentViewModel()
                                  .deleteComment(widget.commentModel.id);
                              if (result) {
                                commentViewModel.commentListModel
                                    .remove(widget.commentModel);
                        ...
                        GestureDetector(
                          onTap: () async {
                            await Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      EditComment(widget.commentModel),
                                ));
                            setState(() {});
                          },
                          child: Text("Sửa"),
  ...
  Widget UserReply() {
    return ScopedModel(
      ...
      child: ScopedModelDescendant(
        builder: (BuildContext context, Widget child, CommentViewModel model) {
                ...
                child: Padding(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      Text(
                          "${DateTimeConvert.timeAgoInShort(commentModel.createdTime)}"),
                      SizedBox(
                        width: 15,
                      ),
                      Text(
                        commentModel.totalReaction == 0
                            ? ""
                            : commentModel.totalReaction.toString(),
                        style: TextStyle(
                            color: commentModel.idReaction != 0
                                ? PINK_COLOR
                                : LIGHT_DARK_GREY_COLOR),
                      ),
                      SizedBox(
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (commentModel.idReaction != 0) {
                            await ReactionViewModel()
                                .deleteReaction(commentModel.idReaction);
                            await ReactionViewModel()
                                .checkCommentReaction(commentModel);
                            commentModel.totalReaction--;
                          } else {
                            await ReactionViewModel()
                                .addCommentReaction(commentModel.id);
                            await ReactionViewModel()
                                .checkCommentReaction(commentModel);
                            commentModel.totalReaction++;
                          }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "Bình luận chưa có phản hồi nào",
        ...
        title: TextFormField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: IconButton(
                icon: Icon(
                  Icons.send_rounded,
                  color: commentFlag == true ? PINK_COLOR : GREY_COLOR,
                  size: 28,
                ),
                onPressed: () async {
