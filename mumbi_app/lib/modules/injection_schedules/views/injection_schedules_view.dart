...
import 'injection_detail_view.dart';
import 'injection_update_phone_view.dart';
...
class _InjectionScheduleState extends State<InjectionSchedule> {
  ...
  void initState() {
    ...
    InjectionSchedulesRepository.getToken().then((value) async {
      if (value != null && value != "") {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        actions: [
          ...
          if (isLogin)
            IconButton(
              ...
              onPressed: () {
                showConfirmDialog(context, "Đăng xuất",
                    "Bạn có muốn đăng xuất khỏi số điện thoại hiện tại",
                    ContinueFunction: () {
      ...
      body: Column(
        children: <Widget>[
          CardInfo(
              childViewModel.childModel.imageURL,
  ...
  void login() async {
    ...
    dialog.show();
    var data = await InjectionSchedulesRepository.login(
    ...
    if (success) {
      ...
      setState(() {});
      await InjectionSchedulesRepository.personListSynchronization();
      await InjectionSchedulesRepository.historyListSynchronization();
      dialog.hide();
      showCustomProgressDialog(context, "Đang lấy dữ liệu",
          injectionScheduleViewModel.getInjectionSchedule(CurrentMember.id),
          (data) {
